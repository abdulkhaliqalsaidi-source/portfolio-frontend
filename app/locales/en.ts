export default {
  // Navigation
  nav: {
    home: 'Home',
    about: 'About',
    services: 'Services',
    skills: 'Skills',
    projects: 'Projects',
    testimonials: 'Testimonials',
    experience: 'Experience',
    blog: 'Blog',
    contact: 'Contact',
    resume: 'Resume / CV',
    downloadCv: 'Download Resume (PDF)',
    resumePdf: 'Resume PDF',
    contactMe: 'Get In Touch',
    adminPanel: 'Admin Dashboard',
    copyEmail: 'Copy Email Address',
    copied: 'Copied!',
    availableForWork: 'Available for Work',
    currentlyBusy: 'Currently Booked',
    availableTooltip: 'Open to freelance contracts & remote engineering roles',
    busyTooltip: 'Currently focused on ongoing enterprise contracts',
    switchThemeLight: 'Switch to Light Mode',
    switchThemeDark: 'Switch to Dark Mode',
    switchLang: 'التحويل إلى العربية'
  },

  // Hero Section
  hero: {
    badge: 'Senior Software Engineer • Full Stack & Frontend Specialist',
    greeting: "Hello, I'm",
    tagline: 'Crafting resilient, high-performance web applications and cloud architectures with state-of-the-art aesthetics and frictionless user experience.',
    exploreProjects: 'Explore Projects',
    contactMe: 'Contact Me',
    liveBadgeText: 'Available for New Projects'
  },

  // Stats & About Section
  stats: {
    eyebrow: 'Vision & Engineering Philosophy • About',
    title: 'Delivering Robust Digital Products',
    titleHighlight: 'with High Performance & Precision',
    sub: 'An engineering approach centered on clean architecture, strict code quality, scalable system design, and outstanding user satisfaction.',
    role: 'Senior Full Stack Software Engineer',
    downloadCv: 'Download Resume (CV)',
    requestConsultation: 'Request Consultation',
    verifiedAchievement: 'Verified Milestone',
    coreCompetencies: 'Core Competencies & Tech Stack',
    competenciesDesc: 'Professional proficiency benchmarks across frontend, backend, databases, and DevOps tools.',
    verifiedSkills: 'Verified Skills'
  },

  // Services Section
  services: {
    eyebrow: 'Engineering Services • What I Do',
    title: 'End-to-End Technical Solutions',
    titleHighlight: 'from Concept to Production',
    sub: 'Comprehensive engineering services covering the complete digital product lifecycle using modern stacks and cloud best practices.'
  },

  // Projects Section
  projects: {
    eyebrow: 'Featured Work • Portfolio & Case Studies',
    title: 'Battle-Tested Software Solutions',
    titleHighlight: 'Built & Shipped to Production',
    sub: 'Selected enterprise platforms, GIS spatial tools, and web applications built with uncompromising performance, reliability, and security standards.',
    allFilter: 'All',
    readCaseStudy: 'Read Interactive Case Study',
    liveDemo: 'Live Demo',
    repository: 'Source Code',
    viewAllProjects: 'View All Projects',
    beforeAfter: 'Before & After Transformation',
    beforeLabel: 'Initial Draft & Prototype',
    afterLabel: 'Final Production Solution',
    dragHint: 'Drag to Compare'
  },

  // Testimonials Section
  testimonials: {
    eyebrow: 'Client Endorsements • Testimonials',
    title: 'What Collaborators & Clients Say',
    titleHighlight: 'About Working Together',
    sub: 'Direct feedback and reviews from project managers, founders, and technical partners.'
  },

  // Timeline / Experience Section
  timeline: {
    eyebrow: 'Career Journey • Milestones & Growth',
    title: 'Engineering Experience',
    titleHighlight: '& Professional Timeline',
    sub: 'Chronological timeline of leadership roles, enterprise deliveries, and accredited certifications.'
  },

  // Contact Section
  contact: {
    eyebrow: "Let's Connect • Start a Project",
    title: "Let's Turn Your Vision Into a",
    titleHighlight: 'Remarkable Digital Product',
    sub: 'Open to contract engagements, freelance projects, and technical consulting. Reach out and let’s discuss your objectives.',
    nameLabel: 'Full Name *',
    namePlaceholder: 'Your name or company name',
    emailLabel: 'Email Address *',
    emailPlaceholder: 'name@example.com',
    phoneLabel: 'Phone Number (Optional)',
    phonePlaceholder: '+1 ... / +967 ...',
    subjectLabel: 'Subject *',
    subjectPlaceholder: 'Web Application Development / Consulting',
    messageLabel: 'Project Details & Requirements *',
    messagePlaceholder: 'Tell me about the project scope, technical requirements, and estimated timeline...',
    sendBtn: 'Send Message',
    sending: 'Sending...',
    successMsg: 'Thank you! Your message has been sent successfully. I will get back to you shortly.',
    errorMsg: 'An error occurred while sending your message. Please try again or reach out via email directly.'
  },

  // Tech Stack Section
  stack: {
    eyebrow: 'Skills & Tech Stack • Technical Arsenal',
    title: 'Technical Competencies',
    titleHighlight: '& Core Technologies',
    sub: 'Core engineering capabilities, methodologies, and framework competencies proven across demanding production environments.',
    skillsCount: 'Skills'
  },

  // Blog Section
  blog: {
    eyebrow: 'Insights & Technical Writings • Blog',
    title: 'Engineering Blog',
    titleHighlight: '& Articles',
    sub: 'Hands-on architectural insights, development best practices, and deep dives into modern web platforms.',
    featured: 'Featured',
    minRead: 'min read',
    readMore: 'Read Article',
    allArticles: 'Explore All Articles in Blog'
  },

  // Footer Section
  footer: {
    builtWith: 'Engineered with Nuxt 4, Vue 3 & Django with modern design aesthetics.',
    rights: 'All rights reserved.'
  }
}
